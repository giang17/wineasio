/*
 * Minimal WineASIO test DLL - INLINE GUIDs (no uuid library)
 * Tests if the crash is caused by excessive exports from libuuid
 */
...
#define COBJMACROS
#define INITGUID  /* This makes GUIDs be defined inline */
#include <windows.h>
#include <objbase.h>
...
#define DBG(fmt, ...) do { \
    char _buf[512]; \
    snprintf(_buf, sizeof(_buf), "[MinimalASIO-InlineGUID] " fmt "\n", ##__VA_ARGS__); \
    OutputDebugStringA(_buf); \
    fprintf(stderr, "%s", _buf); \
    fflush(stderr); \
} while(0)

/* Define our own GUIDs inline to avoid libuuid.a exports */
...
{
    (void)iface;
    if (name) strcpy(name, "WineASIO (InlineGUID Test)");
}
...
{
    (void)iface;
    if (string) strcpy(string, "Minimal test - inline GUIDs");
}
...
/* Empty register/unregister - NO advapi32 dependency */
