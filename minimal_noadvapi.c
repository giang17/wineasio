/*
 * Minimal WineASIO test DLL - NO ADVAPI32 (registry) functions
 * Tests if the crash is caused by advapi32.dll imports
 */
...
#define DBG(fmt, ...) do { \
    char _buf[512]; \
    snprintf(_buf, sizeof(_buf), "[MinimalASIO-NoReg] " fmt "\n", ##__VA_ARGS__); \
...
{
    (void)iface;
    if (name) strcpy(name, "WineASIO (NoReg Test)");
}
...
{
    (void)iface;
    if (string) strcpy(string, "Minimal test - no registry");
}
